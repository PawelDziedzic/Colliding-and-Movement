...
public class PrunedPlayerscript : MonoBehaviour {
	...
	void Update () {
		...
		//Add to movement from jump, if isGrounded()
	}
	...
	{
		movement3D = vectorCutOnMargin(getShortestVector(ShorteningFromHead(),getShortestVector(ShorteningFromCore(),ShorteningFromBottom())));
	}

	Vector3 vectorCutOnMargin(Vector3 vec)
	{
		if (vec.magnitude >= 0.01f) {
			return vec-vec.normalized*0.005f;
		} else {
			return Vector3.zero; 
		}
	}
